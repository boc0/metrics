...
@expect(fib, (3, 4, 2))
def test_find_control_structures(root, expected):
    control_dependent, data_dependent, variables = find_control_structures_outer(root)
    assert control_dependent == expected[0]
    assert data_dependent == expected[1]
    assert variables == expected[2]
