...
def expect(code, res, fixture='root'):
    """Wrapper for pytest.mark.parametrize which gives test functions
    the examples to test on and expected results"""
    return lambda func: pytest.mark.parametrize(f'{fixture}, expected', [(code, res)], indirect=[fixture])(func)
...
def test_n_nested_loops():
    assert n_nested_loops(loops) == 9

def test_max_nesting_level_of_loops():
    assert max_nesting_level_of_loops(loops) == 5
...
def test_n_param_variables():
    assert n_param_variables(params) == 3

def test_n_variables_as_parameters():
    assert n_variables_as_parameters(params) == 3
...
def test_max_nesting_level_of_control_structures():
    assert max_nesting_level_of_control_structures(fib) == 2

def test_control_dependent_control_structures():
    assert control_dependent_control_structures(fib) == 3
...
def test_param_names(root, expected):
    funcdef = root.children[0]
    assert param_names(get_parameters(funcdef)) == expected
